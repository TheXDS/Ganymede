...
public interface IDialogTemplateBuilder<in T> : IDialogTemplateBuilder where T : IDialogViewModel
{
    ...
    /// </param>
    /// <returns>
    /// A <see cref="StyledElement"/> that allows for interactivity with the
    /// specified <see cref="DialogViewModel"/>.
    /// </returns>
    StyledElement? Build(T viewModel);
    ...
    /// </param>
    /// <returns>
    /// A <see cref="StyledElement"/> that defines the dialog UI for the
    /// specified <see cref="IDialogViewModel"/>, or <see langword="null"/> if
    /// no UI was generated.
    /// </returns>
    StyledElement? IDialogTemplateBuilder.Build(IDialogViewModel vm) => Build((T)vm);
}
